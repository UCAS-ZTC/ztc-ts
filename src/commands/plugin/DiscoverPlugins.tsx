...
export function DiscoverPlugins({
      ...
      try {
        ...
        if (errorResult) {
          if (errorResult.type === 'warning') {
            setWarning(errorResult.message + uiText('. Showing available plugins.', '。将显示可用插件。'));
          } else {
        ...
        if (targetPlugin) {
          ...
          if (foundPlugin) {
            if (foundPlugin.isInstalled) {
              setError(uiText(`Plugin '${foundPlugin.pluginId}' is already installed. Use '/plugin' to manage existing plugins.`, `插件 '${foundPlugin.pluginId}' 已安装。请使用 '/plugin' 管理已安装插件。`));
            } else {
              setSelectedPlugin(foundPlugin);
              setViewState('plugin-details');
            }
          } else {
            setError(uiText(`Plugin "${targetPlugin}" not found in any marketplace`, `在所有插件市场中都未找到插件 "${targetPlugin}"`));
          }
        }
      } catch (err) {
        setError(err instanceof Error ? err.message : uiText('Failed to load plugins', '加载插件失败'));
      } finally {
        setLoading(false);
      }
    }
  ...
  const installSelectedPlugins = async () => {
    ...
    if (failureCount === 0) {
      const message = uiText(`✓ Installed ${successCount_0} ${plural(successCount_0, 'plugin')}. Run /reload-plugins to activate.`, `✓ 已安装 ${successCount_0} 个插件。请运行 /reload-plugins 激活。`);
      setResult(message);
    } else if (successCount_0 === 0) {
      setError(uiText(`Failed to install: ${formatFailureDetails(newFailedPlugins, true)}`, `安装失败：${formatFailureDetails(newFailedPlugins, true)}`));
    } else {
      const message_0 = uiText(`✓ Installed ${successCount_0} of ${successCount_0 + failureCount} plugins. Failed: ${formatFailureDetails(newFailedPlugins, false)}. Run /reload-plugins to activate successfully installed plugins.`, `✓ 共 ${successCount_0 + failureCount} 个插件中已安装 ${successCount_0} 个。失败：${formatFailureDetails(newFailedPlugins, false)}。请运行 /reload-plugins 激活已成功安装的插件。`);
      setResult(message_0);
    }
    if (successCount_0 > 0) {
  ...
  if (typeof viewState === 'object' && viewState.type === 'plugin-options') {
    ...
    return <PluginOptionsFlow plugin={plugin_4} pluginId={pluginId_0} onDone={(outcome, detail) => {
      switch (outcome) {
        case 'configured':
          finish(uiText(`✓ Installed and configured ${plugin_4.name}. Run /reload-plugins to apply.`, `✓ 已安装并完成配置 ${plugin_4.name}。请运行 /reload-plugins 生效。`));
          break;
        case 'skipped':
          finish(uiText(`✓ Installed ${plugin_4.name}. Run /reload-plugins to apply.`, `✓ 已安装 ${plugin_4.name}。请运行 /reload-plugins 生效。`));
          break;
        case 'error':
          finish(uiText(`Installed but failed to save config: ${detail}`, `已安装，但保存配置失败：${detail}`));
          break;
      }
    }} />;
  }
  ...
  // Loading state
  if (loading) {
    return <Text>{uiText('Loading…', '加载中…')}</Text>;
  }
  ...
  if (viewState === 'plugin-details' && selectedPlugin) {
    ...
    return <Box flexDirection="column">
        <Box marginBottom={1}>
          <Text bold>{uiText('Plugin details', '插件详情')}</Text>
        </Box>

        <Box flexDirection="column" marginBottom={1}>
          <Text bold>{selectedPlugin.entry.name}</Text>
          <Text dimColor>{uiText('from ', '来源：')}{selectedPlugin.marketplaceName}</Text>
          {selectedPlugin.entry.version && <Text dimColor>{uiText('Version: ', '版本：')}{selectedPlugin.entry.version}</Text>}
        ...
        {installError && <Box marginBottom={1}>
            <Text color="error">{uiText('Error: ', '错误：')}{installError}</Text>
          </Box>}
        ...
        <Box flexDirection="column">
          {menuOptions.map((option, index) => <Box key={option.action}>
              ...
              <Text bold={detailsMenuIndex === index}>
                {isInstalling && option.action.startsWith('install-') ? uiText('Installing…', '安装中…') : option.label}
              </Text>
            </Box>)}
        </Box>
  ...
  if (availablePlugins.length === 0) {
    return <Box flexDirection="column">
        <Box marginBottom={1}>
          <Text bold>{uiText('Discover plugins', '发现插件')}</Text>
        </Box>
        <EmptyStateMessage reason={emptyReason} />
        <Box marginTop={1}>
          <Text dimColor italic>
            {uiText('Esc to go back', '按 Esc 返回')}
          </Text>
        </Box>
      </Box>;
  }
  ...
  return <Box flexDirection="column">
      ...
      {filteredPlugins.length === 0 && searchQuery && <Box marginBottom={1}>
          <Text dimColor>{uiText('No plugins match ', '没有插件匹配 ')}&quot;{searchQuery}&quot;</Text>
        </Box>}
      ...
      return <Box key={`${pagination.startIndex}-${plugin_5.pluginId}`} flexDirection="column" marginBottom={isLast && !error ? 0 : 1}>
            <Box>
              ...
              <Text>
                ...
                {plugin_5.entry.tags?.includes('community-managed') && <Text dimColor> [{uiText('Community Managed', '社区维护')}]</Text>}
...
export function DiscoverPluginsKeyHint(t0) {
  const $ = _c(10);
  const {
    hasSelection,
    canToggle
  ...
  let t1;
  if ($[0] !== hasSelection) {
    t1 = hasSelection && <ConfigurableShortcutHint action="plugin:install" context="Plugin" fallback="i" description={uiText('install', '安装')} bold={true} />;
    $[0] = hasSelection;
    $[1] = t1;
  } else {
    t1 = $[1];
  ...
  if ($[2] === Symbol.for("react.memo_cache_sentinel")) {
    t2 = <Text>{uiText('type to search', '输入即可搜索')}</Text>;
    $[2] = t2;
  } else {
    t2 = $[2];
  }
  let t3;
  if ($[3] !== canToggle) {
    t3 = canToggle && <ConfigurableShortcutHint action="plugin:toggle" context="Plugin" fallback="Space" description={uiText('toggle', '切换')} />;
    $[3] = canToggle;
    $[4] = t3;
  } else {
    t3 = $[4];
  ...
  if ($[5] === Symbol.for("react.memo_cache_sentinel")) {
    t4 = <ConfigurableShortcutHint action="select:accept" context="Select" fallback="Enter" description={uiText('details', '详情')} />;
    t5 = <ConfigurableShortcutHint action="confirm:no" context="Confirmation" fallback="Esc" description={uiText('back', '返回')} />;
    $[5] = t4;
    $[6] = t5;
  } else {
    t4 = $[5];
 ...
 */
export function EmptyStateMessage(t0) {
  const $ = _c(6);
  const {
    reason
  } = t0;
    ...
    case "git-not-installed":
      {
        ...
        if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
          t1 = <><Text dimColor={true}>{uiText('Git is required to install marketplaces.', '安装插件市场需要 Git。')}</Text><Text dimColor={true}>{uiText('Please install git and restart Claude Code.', '请安装 Git 后重启 Claude Code。')}</Text></>;
          $[0] = t1;
        } else {
          t1 = $[0];
        }
    ...
    case "all-blocked-by-policy":
      {
        ...
        if ($[1] === Symbol.for("react.memo_cache_sentinel")) {
          t1 = <><Text dimColor={true}>{uiText('Your organization policy does not allow any external marketplaces.', '你的组织策略不允许任何外部插件市场。')}</Text><Text dimColor={true}>{uiText('Contact your administrator.', '请联系你的管理员。')}</Text></>;
          $[1] = t1;
        } else {
          t1 = $[1];
        }
    ...
    case "policy-restricts-sources":
      {
        let t1;
        if ($[2] === Symbol.for("react.memo_cache_sentinel")) {
          t1 = <><Text dimColor={true}>{uiText('Your organization restricts which marketplaces can be added.', '你的组织限制了可添加的插件市场来源。')}</Text><Text dimColor={true}>{uiText('Switch to the Marketplaces tab to view allowed sources.', '请切换到“插件市场”页签查看允许的来源。')}</Text></>;
          $[2] = t1;
        } else {
          t1 = $[2];
        }
    ...
    case "all-marketplaces-failed":
      {
        ...
        if ($[3] === Symbol.for("react.memo_cache_sentinel")) {
          t1 = <><Text dimColor={true}>{uiText('Failed to load marketplace data.', '加载插件市场数据失败。')}</Text><Text dimColor={true}>{uiText('Check your network connection.', '请检查网络连接。')}</Text></>;
          $[3] = t1;
        } else {
          t1 = $[3];
        }
    ...
    case "all-plugins-installed":
      {
        ...
        if ($[4] === Symbol.for("react.memo_cache_sentinel")) {
          t1 = <><Text dimColor={true}>{uiText('All available plugins are already installed.', '所有可用插件都已安装。')}</Text><Text dimColor={true}>{uiText('Check for new plugins later or add more marketplaces.', '你可以稍后再检查，或添加更多插件市场。')}</Text></>;
          $[4] = t1;
        } else {
          t1 = $[4];
        }
    ...
    default:
      {
        let t1;
        if ($[5] === Symbol.for("react.memo_cache_sentinel")) {
          t1 = <><Text dimColor={true}>{uiText('No plugins available.', '当前没有可用插件。')}</Text><Text dimColor={true}>{uiText('Add a marketplace first using the Marketplaces tab.', '请先在“插件市场”页签添加插件市场。')}</Text></>;
          $[5] = t1;
        } else {
          t1 = $[5];
        }
