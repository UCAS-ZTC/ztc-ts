...
export function ManageMarketplaces({
      ...
      try {
        ...
        if (targetMarketplace && !hasAttemptedAutoAction.current && !error) {
          ...
          if (targetIndex >= 0) {
            ...
            setError(uiText(`Marketplace not found: ${targetMarketplace}`, `未找到插件市场：${targetMarketplace}`));
          }
        }
      } catch (err) {
        if (setError) {
          setError(err instanceof Error ? err.message : uiText('Failed to load marketplaces', '加载插件市场失败'));
        }
        setProcessError(err instanceof Error ? err.message : uiText('Failed to load marketplaces', '加载插件市场失败'));
      } finally {
        setLoading(false);
      }
    }
  ...
  const applyChanges = async (states?: MarketplaceState[]) => {
    ...
    try {
      ...
      if (updatedCount > 0) {
        const pluginPart = updatedPluginCount > 0 ? uiText(` (${updatedPluginCount} ${plural(updatedPluginCount, 'plugin')} bumped)`, `（已同步升级 ${updatedPluginCount} 个插件）`) : '';
        actions.push(uiText(`Updated ${updatedCount} ${plural(updatedCount, 'marketplace')}${pluginPart}`, `已更新 ${updatedCount} 个插件市场${pluginPart}`));
      }
      if (removedCount > 0) {
        actions.push(uiText(`Removed ${removedCount} ${plural(removedCount, 'marketplace')}`, `已移除 ${removedCount} 个插件市场`));
      }
      if (actions.length > 0) {
  ...
  const buildDetailsMenuOptions = (marketplace: MarketplaceState | null): Array<{
    ...
    const options: Array<{
      ...
      label: uiText(`Browse plugins (${marketplace.pluginCount ?? 0})`, `浏览插件（${marketplace.pluginCount ?? 0}）`),
      value: 'browse'
    }, {
      label: uiText('Update marketplace', '更新插件市场'),
      secondaryLabel: marketplace.lastUpdated ? uiText(`(last updated ${new Date(marketplace.lastUpdated).toLocaleDateString()})`, `（最近更新：${new Date(marketplace.lastUpdated).toLocaleDateString()}）`) : undefined,
      value: 'update'
    }];
    ...
    if (!shouldSkipPluginAutoupdate()) {
      options.push({
        label: marketplace.autoUpdate ? uiText('Disable auto-update', '关闭自动更新') : uiText('Enable auto-update', '开启自动更新'),
        value: 'toggle-auto-update'
      });
    }
    options.push({
      label: uiText('Remove marketplace', '移除插件市场'),
      value: 'remove'
    });
    return options;
  };
  ...
  const handleToggleAutoUpdate = async (marketplace: MarketplaceState) => {
    ...
    try {
      ...
      setProcessError(err instanceof Error ? err.message : uiText('Failed to update setting', '更新设置失败'));
    }
  };
  ...
  if (loading) {
    return <Text>{uiText('Loading marketplaces…', '正在加载插件市场…')}</Text>;
  }
  if (marketplaceStates.length === 0) {
    return <Box flexDirection="column">
        <Box marginBottom={1}>
          <Text bold>{uiText('Manage marketplaces', '管理插件市场')}</Text>
        </Box>
        ...
        <Box flexDirection="row" gap={1}>
          ...
          <Text bold color="suggestion">
            {uiText('Add Marketplace', '添加插件市场')}
          </Text>
        </Box>

        <Box marginLeft={3}>
          <Text dimColor italic>
            {exitState.pending ? <>{uiText('Press ', '再次按 ')}{exitState.keyName}{uiText(' again to go back', ' 返回')}</> : <Byline>
                <ConfigurableShortcutHint action="select:accept" context="Select" fallback="Enter" description={uiText('select', '选择')} />
                <ConfigurableShortcutHint action="confirm:no" context="Confirmation" fallback="Esc" description={uiText('go back', '返回')} />
              </Byline>}
          </Text>
        </Box>
      </Box>;
  ...
  if (internalView === 'confirm-remove' && selectedMarketplace) {
    ...
    return <Box flexDirection="column">
        <Text bold color="warning">
          {uiText('Remove marketplace ', '移除插件市场 ')}<Text italic>{selectedMarketplace.name}</Text>?
        </Text>
        <Box flexDirection="column">
          {pluginCount > 0 && <Box marginTop={1}>
              <Text color="warning">
                {uiText('This will also uninstall ', '这还会卸载来自该插件市场的 ')}{pluginCount}{' '}
                {uiText(plural(pluginCount, 'plugin'), '个插件')}:
              </Text>
            </Box>}
          ...
          <Box marginTop={1}>
            <Text>
              {uiText('Press ', '按 ')}<Text bold>y</Text>{uiText(' to confirm or ', ' 确认，按 ')}<Text bold>n</Text>{uiText(' to cancel', ' 取消')}
            </Text>
          </Box>
        </Box>
      </Box>;
  ...
  if (internalView === 'details' && selectedMarketplace) {
    ...
    return <Box flexDirection="column">
        ...
        <Box marginTop={1}>
          <Text>
            {selectedMarketplace.pluginCount || 0} {uiText('available', '可用')}{' '}
            {uiText(plural(selectedMarketplace.pluginCount || 0, 'plugin'), '插件')}
          </Text>
        </Box>
        ...
        {selectedMarketplace.installedPlugins && selectedMarketplace.installedPlugins.length > 0 && <Box flexDirection="column" marginTop={1}>
              <Text bold>
                {uiText('Installed plugins', '已安装插件')} ({selectedMarketplace.installedPlugins.length}
                )：
              </Text>
        ...
        {isUpdating && <Box marginTop={1} flexDirection="column">
            <Text color="claude">{uiText('Updating marketplace…', '正在更新插件市场…')}</Text>
        ...
        {!isUpdating && !shouldSkipPluginAutoupdate() && selectedMarketplace.autoUpdate && <Box marginTop={1}>
              <Text dimColor>
                {uiText('Auto-update enabled. Claude Code will automatically update this marketplace and its installed plugins.', '已启用自动更新。Claude Code 会自动更新该插件市场及其已安装插件。')}
              </Text>
            </Box>}

        <Box marginLeft={3}>
          <Text dimColor italic>
            {isUpdating ? <>{uiText('Please wait…', '请稍候…')}</> : <Byline>
                <ConfigurableShortcutHint action="select:accept" context="Select" fallback="Enter" description={uiText('select', '选择')} />
                <ConfigurableShortcutHint action="confirm:no" context="Confirmation" fallback="Esc" description={uiText('go back', '返回')} />
              </Byline>}
          </Text>
  ...
  return <Box flexDirection="column">
      ...
      <Box flexDirection="column">
        ...
        if (state.pendingUpdate) indicators.push(uiText('UPDATE', '待更新'));
        if (state.pendingRemove) indicators.push(uiText('REMOVE', '待移除'));
        return <Box key={state.name} flexDirection="row" gap={1} marginBottom={1}>
              ...
              <Box flexDirection="column" flexGrow={1}>
                ...
                <Text dimColor>
                  {state.pluginCount !== undefined && <>{state.pluginCount} {uiText('available', '可用')}</>}
                  {state.installedPlugins && state.installedPlugins.length > 0 && <> • {state.installedPlugins.length} {uiText('installed', '已安装')}</>}
                  {state.lastUpdated && <>
                      {' '}
                      • {uiText('Updated', '更新于')}{' '}
      ...
      {hasPendingChanges() && <Box marginTop={1} flexDirection="column">
          <Text>
            <Text bold>{uiText('Pending changes:', '待处理变更：')}</Text>{' '}
            <Text dimColor>{uiText('Enter to apply', '按 Enter 应用')}</Text>
          </Text>
          {updateCount > 0 && <Text>
              • {uiText('Update', '更新')} {updateCount} {uiText(plural(updateCount, 'marketplace'), '个插件市场')}
            </Text>}
          {removeCount > 0 && <Text color="warning">
              • {uiText('Remove', '移除')} {removeCount} {uiText(plural(removeCount, 'marketplace'), '个插件市场')}
            </Text>}
        </Box>}

      {/* Processing indicator */}
      {isProcessing && <Box marginTop={1}>
          <Text color="claude">{uiText('Processing changes…', '正在处理变更…')}</Text>
        </Box>}
...
export function ManageMarketplacesKeyHints({
  exitState,
  hasPendingActions
  ...
  return <Box marginTop={1}>
      <Text dimColor italic>
        <Byline>
          {hasPendingActions ? <ConfigurableShortcutHint action="select:accept" context="Select" fallback="Enter" description={uiText('apply changes', '应用变更')} /> : <ConfigurableShortcutHint action="select:accept" context="Select" fallback="Enter" description={uiText('select', '选择')} />}
          {!hasPendingActions && <KeyboardShortcutHint shortcut="u" action={uiText('update', '更新')} />}
          {!hasPendingActions && <KeyboardShortcutHint shortcut="r" action={uiText('remove', '移除')} />}
          <ConfigurableShortcutHint action="confirm:no" context="Confirmation" fallback="Esc" description={hasPendingActions ? uiText('cancel', '取消') : uiText('go back', '返回')} />
        </Byline>
      </Text>
    </Box>;
}
