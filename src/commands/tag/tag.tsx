...
function ConfirmRemoveTag(t0) {
  ...
  const t1 = uiText(`Current tag: #${tagName}`, `当前标签：#${tagName}`);
  let t2;
  if ($[0] === Symbol.for("react.memo_cache_sentinel")) {
    t2 = <Text>{uiText('This will remove the tag from the current session.', '这将从当前会话中移除该标签。')}</Text>;
    $[0] = t2;
  } else {
    t2 = $[0];
  }
  ...
  if ($[4] === Symbol.for("react.memo_cache_sentinel")) {
    t4 = [{
      label: uiText('Yes, remove tag', '是，移除标签'),
      value: "yes"
    }, {
      label: uiText('No, keep tag', '否，保留标签'),
      value: "no"
    }];
    $[4] = t4;
  } else {
  ...
  if ($[7] !== onCancel || $[8] !== t1 || $[9] !== t5) {
    t6 = <Dialog title={uiText('Remove tag?', '移除标签？')} subtitle={t1} onCancel={onCancel} color="warning">{t5}</Dialog>;
    $[7] = onCancel;
    $[8] = t1;
    $[9] = t5;
    $[10] = t6;
...
function ToggleTagAndClose(t0) {
  ...
  if ($[2] !== normalizedTag || $[3] !== onDone) {
    t2 = () => {
      ...
      if (!id) {
        onDone(uiText('No active session to tag', '当前没有可打标签的会话'), {
          display: "system"
        });
        return;
      }
      if (!normalizedTag) {
        onDone(uiText('Tag name cannot be empty', '标签名不能为空'), {
          display: "system"
        });
        return;
      }
      ...
      if (currentTag === normalizedTag) {
        ...
        (async () => {
          ...
          onDone(uiText(`Tagged session with ${chalk.cyan(`#${normalizedTag}`)}`, `已为会话设置标签 ${chalk.cyan(`#${normalizedTag}`)}`), {
            display: "system"
          });
        })();
      }
  ...
  if (showConfirm && sessionId) {
    ...
    if ($[6] !== normalizedTag || $[7] !== onDone || $[8] !== sessionId) {
      t4 = async () => {
        ...
        onDone(uiText(`Removed tag ${chalk.cyan(`#${normalizedTag}`)}`, `已移除标签 ${chalk.cyan(`#${normalizedTag}`)}`), {
          display: "system"
        });
      };
      $[6] = normalizedTag;
    ...
    if ($[10] !== normalizedTag || $[11] !== onDone) {
      t5 = () => {
        ...
        onDone(uiText(`Kept tag ${chalk.cyan(`#${normalizedTag}`)}`, `已保留标签 ${chalk.cyan(`#${normalizedTag}`)}`), {
          display: "system"
        });
      };
      $[10] = normalizedTag;
