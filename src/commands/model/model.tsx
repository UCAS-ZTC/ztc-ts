...
function ModelPickerWrapper(t0) {
  ...
  if ($[3] !== isFastMode || $[4] !== mainLoopModel || $[5] !== onDone || $[6] !== setAppState) {
    t2 = function handleSelect(model, effort) {
      ...
      if (effort !== undefined) {
        message = message + `，强度: ${chalk.bold(effort)}`;
      }
  ...
  React.useEffect(() => {
    async function handleModelChange(): Promise<void> {
      ...
      try {
        ...
        onDone(`模型验证失败: ${(error as Error).message}`, {
          display: 'system'
        });
      }
    }
...
function ShowModelAndClose(t0) {
  ...
  const displayModel = renderModelLabel(mainLoopModel);
  const effortInfo = effortValue !== undefined ? ` （强度: ${effortValue}）` : "";
...
export const call: LocalJSXCommandCall = async (onDone, _context, args) => {
  ...
  if (COMMON_HELP_ARGS.includes(args)) {
    onDone('运行 /model 打开模型选择菜单，或 /model [模型名] 设置模型。', {
      display: 'system'
    });
    return;
  }
