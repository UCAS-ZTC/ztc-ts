...
async function checkMcpServerHealth(name: string, server: ScopedMcpServerConfig): Promise<string> {
  try {
    ...
    if (result.type === 'connected') {
      return uiText('✓ Connected', '✓ 已连接');
    } else if (result.type === 'needs-auth') {
      return uiText('! Needs authentication', '! 需要认证');
    } else {
      return uiText('✗ Failed to connect', '✗ 连接失败');
    }
  } catch (_error) {
    return uiText('✗ Connection error', '✗ 连接错误');
  }
}
...
export async function mcpServeHandler({
  ...
  try {
    ...
    if (isFsInaccessible(error)) {
      cliError(
        uiText(
          `Error: Directory ${providedCwd} does not exist`,
          `错误：目录 ${providedCwd} 不存在`,
        ),
      );
    }
    throw error;
...
export async function mcpRemoveHandler(name: string, options: {
  ...
  try {
    ...
    if (scopes.length === 0) {
      cliError(uiText(`No MCP server found with name: "${name}"`, `未找到名为 "${name}" 的 MCP 服务器`));
  ...
  if (Object.keys(configs).length === 0) {
    ...
    console.log(
      uiText(
        ...
        '尚未配置 MCP 服务器。使用 `claude mcp add` 添加服务器。',
      ),
    );
  } else {
    // biome-ignore lint/suspicious/noConsole:: intentional console output
    console.log(uiText('Checking MCP server health...\n', '正在检查 MCP 服务器健康状态...\n'));
...
export async function mcpGetHandler(name: string): Promise<void> {
  ...
  console.log(`${name}:`);
  // biome-ignore lint/suspicious/noConsole:: intentional console output
  console.log(`${uiText('  Scope', '  作用域')}: ${getScopeLabel(server.scope)}`);
  ...
  if (server.type === 'sse') {
    // biome-ignore lint/suspicious/noConsole:: intentional console output
    console.log(`${uiText('  Type', '  类型')}: sse`);
    // biome-ignore lint/suspicious/noConsole:: intentional console output
    console.log(`${uiText('  URL', '  地址')}: ${server.url}`);
    if (server.headers) {
      // biome-ignore lint/suspicious/noConsole:: intentional console output
      console.log(uiText('  Headers:', '  请求头:'));
    ...
    if (server.oauth?.clientId || server.oauth?.callbackPort) {
      ...
      if (server.oauth.clientId) {
        parts.push(uiText('client_id configured', '已配置 client_id'));
        const clientConfig = getMcpClientConfig(name, server);
        if (clientConfig?.clientSecret) {
          parts.push(uiText('client_secret configured', '已配置 client_secret'));
        }
      }
      if (server.oauth.callbackPort) {
        parts.push(
          uiText(
            `callback_port ${server.oauth.callbackPort}`,
            `回调端口 ${server.oauth.callbackPort}`,
          ),
        );
      }
      // biome-ignore lint/suspicious/noConsole:: intentional console output
      console.log(`${uiText('  OAuth', '  OAuth 配置')}: ${parts.join(', ')}`);
    }
  } else if (server.type === 'http') {
    // biome-ignore lint/suspicious/noConsole:: intentional console output
    console.log(`${uiText('  Type', '  类型')}: http`);
    // biome-ignore lint/suspicious/noConsole:: intentional console output
    console.log(`${uiText('  URL', '  地址')}: ${server.url}`);
    if (server.headers) {
      // biome-ignore lint/suspicious/noConsole:: intentional console output
    ...
    // biome-ignore lint/suspicious/noConsole:: intentional console output
    console.log(`${uiText('  Type', '  类型')}: stdio`);
    // biome-ignore lint/suspicious/noConsole:: intentional console output
    console.log(`${uiText('  Command', '  命令')}: ${server.command}`);
    const args = Array.isArray(server.args) ? server.args : [];
    // biome-ignore lint/suspicious/noConsole:: intentional console output
    console.log(`${uiText('  Args', '  参数')}: ${args.join(' ')}`);
    if (server.env) {
      // biome-ignore lint/suspicious/noConsole:: intentional console output
      console.log(uiText('  Environment:', '  环境变量:'));
  ...
  console.log(
    uiText(
      ...
      `\n如需移除此服务器，请运行：claude mcp remove "${name}" -s ${server.scope}`,
    ),
  );
  // Use gracefulShutdown to properly clean up MCP server connections
  // (process.exit bypasses cleanup handlers, leaving child processes orphaned)
...
export async function mcpAddJsonHandler(name: string, json: string, options: {
  ...
  try {
    ...
    cliOk(
      uiText(
        `Added ${transportType} MCP server ${name} to ${scope} config`,
        `已将 ${transportType} MCP 服务器 ${name} 添加到 ${scope} 配置`,
      ),
    );
  } catch (error) {
    cliError((error as Error).message);
    ...
    if (Object.keys(servers).length === 0) {
      cliOk(
        uiText(
          ...
          '未在 Claude Desktop 配置中找到 MCP 服务器，或配置文件不存在。',
        ),
      );
    }
    const {
...
export async function mcpResetChoicesHandler(): Promise<void> {
  ...
  cliOk(
    uiText(
      ...
      '已重置当前项目中所有 project 作用域（.mcp.json）服务器的允许/拒绝记录。\n' +
        '下次启动 Claude Code 时会重新请求你的确认。',
    ),
  );
}
