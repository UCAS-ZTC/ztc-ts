...
export function TrustDialog(t0) {
  ...
  if ($[20] === Symbol.for("react.memo_cache_sentinel")) {
    ...
    t17 = <Text>安全检查：这是你创建的或信任的项目吗？（例如你自己的代码、知名开源项目或团队项目）。如果不是，请先检查此文件夹的内容。</Text>;
    t18 = <Text>Claude Code 将能读取、编辑和执行此目录中的文件。</Text>;
    $[20] = t16;
    $[21] = t17;
    $[22] = t18;
  } else {
  ...
  if ($[27] !== exitState.keyName || $[28] !== exitState.pending) {
    t22 = <Text dimColor={true}>{exitState.pending ? <>再按 {exitState.keyName} 退出</> : <>Enter 确认 · Esc 取消</>}</Text>;
