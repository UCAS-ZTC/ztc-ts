...
export function DiffDialog(t0) {
  ...
  if ($[51] !== dismissShortcut || $[52] !== sources.length || $[53] !== viewMode) {
    t23 = exitState => exitState.pending ? <Text>再按 {exitState.keyName} 退出</Text> : viewMode === "list" ? <Byline>{sources.length > 1 && <Text>←/→ 切换来源</Text>}<Text>↑/↓ 选择</Text><Text>Enter 查看</Text><Text>{dismissShortcut} 关闭</Text></Byline> : <Byline><Text>← 返回</Text><Text>{dismissShortcut} 关闭</Text></Byline>;
    $[51] = dismissShortcut;
    $[52] = sources.length;
    $[53] = viewMode;
    $[54] = t23;
