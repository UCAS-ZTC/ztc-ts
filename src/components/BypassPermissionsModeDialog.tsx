...
export function BypassPermissionsModeDialog(t0) {
  ...
  if ($[3] === Symbol.for("react.memo_cache_sentinel")) {
    t3 = <Box flexDirection="column" gap={1}><Text>在跳过权限模式下，Claude Code 不会在执行潜在危险命令前请求你的批准。<Newline />此模式仅应在网络受限且可轻松恢复的沙箱容器/虚拟机中使用。</Text><Text>继续操作即表示你接受在跳过权限模式下运行的所有责任。</Text><Link url="https://code.claude.com/docs/en/security" /></Box>;
    $[3] = t3;
  } else {
    t3 = $[3];
  }
  ...
  let t5;
  if ($[5] !== onChange) {
    t5 = <Dialog title="警告：Claude Code 正以跳过权限模式运行" color="error" onCancel={handleEscape}>{t3}<Select options={t4} onChange={value_0 => onChange(value_0 as 'accept' | 'decline')} /></Dialog>;
    $[5] = onChange;
    $[6] = t5;
  } else {
    t5 = $[6];
