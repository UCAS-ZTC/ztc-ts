...
function getScopeHeading(scope: ConfigScope): {
  ...
  switch (scope) {
    case 'project':
      return {
        label: uiText('Project MCPs', '项目 MCP'),
        path: describeMcpConfigFilePath(scope)
      };
    case 'user':
      return {
        label: uiText('User MCPs', '用户 MCP'),
        path: describeMcpConfigFilePath(scope)
      };
    case 'local':
      return {
        label: uiText('Local MCPs', '本地 MCP'),
        path: describeMcpConfigFilePath(scope)
      };
    case 'enterprise':
      return {
        label: uiText('Enterprise MCPs', '企业 MCP')
      };
    case 'dynamic':
      return {
        label: uiText('Built-in MCPs', '内置 MCP'),
        path: uiText('always available', '始终可用')
      };
    default:
      return {
        label: scope
...
export function MCPListPanel(t0) {
  ...
  if ($[14] !== onComplete) {
    t7 = () => {
      onComplete(uiText('MCP dialog dismissed', 'MCP 对话框已关闭'), {
        display: "system"
      });
    };
    $[14] = onComplete;
    ...
    t17 = server_3 => {
      ...
      if (server_3.client.type === "disabled") {
        ...
        if (server_3.client.type === "connected") {
          ...
          statusText = uiText('connected', '已连接');
        } else {
          if (server_3.client.type === "pending") {
            ...
            if (reconnectAttempt && maxReconnectAttempts) {
              statusText = uiText(`reconnecting (${reconnectAttempt}/${maxReconnectAttempts})…`, `重连中（${reconnectAttempt}/${maxReconnectAttempts}）…`);
            } else {
              statusText = uiText('connecting…', '连接中…');
            }
          } else {
            if (server_3.client.type === "needs-auth") {
              statusIcon = color("warning", theme)(figures.triangleUpOutline);
              statusText = uiText('needs authentication', '需要认证');
            } else {
              statusIcon = color("error", theme)(figures.cross);
              statusText = uiText('failed', '失败');
            }
          }
        }
      }
  ...
  if ($[41] !== getAgentServerIndex || $[42] !== selectedIndex || $[43] !== theme) {
    t18 = agentServer_1 => {
      ...
      const statusText_0 = agentServer_1.needsAuth ? uiText('may need auth', '可能需要认证') : uiText('agent-only', '仅 Agent');
  ...
  let t20;
  if ($[46] !== totalServers) {
    t20 = uiText(plural(totalServers, 'server'), '个服务器');
    $[46] = totalServers;
    $[47] = t20;
  } else {
    t20 = $[47];
  ...
  if ($[60] !== hasFailedClients) {
    t26 = hasFailedClients && <Text dimColor={true}>{debugMode ? uiText('※ Error logs shown inline with --debug', '※ 错误日志已通过 --debug 内联显示') : uiText('※ Run claude --debug to see error logs', '※ 运行 claude --debug 查看错误日志')}</Text>;
    $[60] = hasFailedClients;
    $[61] = t26;
  } else {
    t26 = $[61];
  ...
  if ($[62] === Symbol.for("react.memo_cache_sentinel")) {
    t27 = <Text dimColor={true}><Link url="https://code.claude.com/docs/en/mcp">https://code.claude.com/docs/en/mcp</Link>{" "}{uiText('for help', '查看帮助')}</Text>;
    $[62] = t27;
  } else {
    t27 = $[62];
  }
  ...
  if ($[71] !== handleCancel || $[72] !== t21 || $[73] !== t29) {
    t30 = <Dialog title={uiText('Manage MCP servers', '管理 MCP 服务器')} subtitle={t21} onCancel={handleCancel} hideInputGuide={true}>{t29}</Dialog>;
    $[71] = handleCancel;
    $[72] = t21;
    $[73] = t29;
    $[74] = t30;
  ...
  if ($[75] === Symbol.for("react.memo_cache_sentinel")) {
    t31 = <Box paddingX={1}><Text dimColor={true} italic={true}><Byline><KeyboardShortcutHint shortcut={"\u2191\u2193"} action={uiText('navigate', '导航')} /><KeyboardShortcutHint shortcut="Enter" action={uiText('confirm', '确认')} /><ConfigurableShortcutHint action="confirm:no" context="Confirmation" fallback="Esc" description={uiText('cancel', '取消')} /></Byline></Text></Box>;
    $[75] = t31;
  } else {
    t31 = $[75];
  }
...
export function MCPListPanelFooter(): React.ReactNode {
  return <Box paddingX={1}><Text dimColor={true} italic={true}><Byline><KeyboardShortcutHint shortcut={"↑↓"} action={uiText('navigate', '导航')} /><KeyboardShortcutHint shortcut="Enter" action={uiText('confirm', '确认')} /><ConfigurableShortcutHint action="confirm:no" context="Confirmation" fallback="Esc" description={uiText('cancel', '取消')} /></Byline></Text></Box>;
}
function _temp6(s_2) {
