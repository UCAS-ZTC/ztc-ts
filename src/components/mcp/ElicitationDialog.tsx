...
function formatDateDisplay(isoValue: string, schema: PrimitiveSchemaDefinition): string {
  try {
    ...
    if (parts.length === 3) {
      ...
      return local.toLocaleDateString('zh-CN', {
        weekday: 'short',
        year: 'numeric',
        month: 'short',
        day: 'numeric'
...
function ElicitationFormDialog({
  ...
  useNotifyAfterTimeout(uiText('Claude Code needs your input', 'Claude Code 需要你的输入'), 'elicitation_dialog');
  ...
  function validateMultiSelect(fieldName: string, schema_0: PrimitiveSchemaDefinition) {
    ...
    if (min !== undefined && selected.length < min && (selected.length > 0 || fieldRequired)) {
      updateValidationError(fieldName, uiText(`Select at least ${min} ${plural(min, 'item')}`, `请至少选择 ${min} 项`));
    } else if (max !== undefined && selected.length > max) {
      updateValidationError(fieldName, uiText(`Select at most ${max} ${plural(max, 'item')}`, `最多只能选择 ${max} 项`));
    } else {
  ...
  function setField(fieldName_0: string, value: number | string | boolean | string[] | undefined) {
    ...
    if (value !== undefined && validationErrors[fieldName_0] === uiText('This field is required', '此字段为必填项')) {
      ...
      if (_input === ' ') {
        ...
        if (optionValue !== undefined) {
          ...
          if (min_0 !== undefined && newSelected.length < min_0 && (newSelected.length > 0 || currentField.isRequired)) {
            updateValidationError(currentField.name, uiText(`Select at least ${min_0} ${plural(min_0, 'item')}`, `请至少选择 ${min_0} 项`));
          } else if (max_0 !== undefined && newSelected.length > max_0) {
            updateValidationError(currentField.name, uiText(`Select at most ${max_0} ${plural(max_0, 'item')}`, `最多只能选择 ${max_0} 项`));
          } else {
      ...
      if (validateRequired() && Object.keys(validationErrors).length === 0) {
        ...
        for (const fieldName_7 of requiredFields_0) {
          if (formValues[fieldName_7] === undefined) {
            updateValidationError(fieldName_7, uiText('This field is required', '此字段为必填项'));
          }
        }
  ...
  function renderFormFields(): React.ReactNode {
    ...
    return <Box flexDirection="column">
        ...
        {hasFieldsBelow && <Box marginLeft={2}>
            <Text dimColor>
              {figures.arrowDown} {uiText(`${schemaFields.length - scrollWindow.end} more below`, `下方还有 ${schemaFields.length - scrollWindow.end} 项`)}
            </Text>
          </Box>}
      </Box>;
  }
  return <Dialog title={uiText(`MCP server \u201c${serverName}\u201d requests your input`, `MCP 服务器“${serverName}”需要你的输入`)} subtitle={`\n${message}`} color="permission" onCancel={() => onResponse('cancel')} isCancelActive={(!currentField || !!focusedButton) && !expandedAccordion} inputGuide={exitState => exitState.pending ? <Text>{uiText(`Press ${exitState.keyName} again to exit`, `再次按 ${exitState.keyName} 退出`)}</Text> : <Byline>
            ...
            {currentField && <KeyboardShortcutHint shortcut="Backspace" action={uiText('unset', '清空')} />}
            {currentField && currentField.schema.type === 'boolean' && <KeyboardShortcutHint shortcut="Space" action={uiText('toggle', '切换')} />}
            {currentField && isEnumSchema(currentField.schema) && (expandedAccordion ? <KeyboardShortcutHint shortcut="Space" action={uiText('select', '选择')} /> : <KeyboardShortcutHint shortcut="→" action={uiText('expand', '展开')} />)}
            {currentField && isMultiSelectEnumSchema(currentField.schema) && (expandedAccordion ? <KeyboardShortcutHint shortcut="Space" action={uiText('toggle', '切换')} /> : <KeyboardShortcutHint shortcut="→" action={uiText('expand', '展开')} />)}
          </Byline>}>
...
function ElicitationURLDialog({
  ...
  useNotifyAfterTimeout(uiText('Claude Code needs your input', 'Claude Code 需要你的输入'), 'elicitation_url_dialog');
  ...
  if (phase === 'waiting') {
    const actionLabel = waitingState?.actionLabel ?? uiText('Continue without waiting', '不再等待，继续执行');
    return <Dialog title={uiText(`MCP server \u201c${serverName}\u201d \u2014 waiting for completion`, `MCP 服务器“${serverName}”正在等待完成`)} subtitle={`\n${message}`} color="permission" onCancel={() => onWaitingDismiss?.('cancel')} isCancelActive inputGuide={exitState => exitState.pending ? <Text>{uiText(`Press ${exitState.keyName} again to exit`, `再次按 ${exitState.keyName} 退出`)}</Text> : <Byline>
              <ConfigurableShortcutHint action="confirm:no" context="Confirmation" fallback="Esc" description={uiText('cancel', '取消')} />
              <KeyboardShortcutHint shortcut="\u2190\u2192" action={uiText('switch', '切换')} />
            </Byline>}>
        <Box flexDirection="column">
          ...
          <Box marginBottom={1}>
            <Text dimColor italic>
              {uiText('Waiting for the server to confirm completion…', '正在等待服务器确认完成…')}
            </Text>
          </Box>
          <Box>
            <Text color="success">
  ...
  return <Dialog title={uiText(`MCP server \u201c${serverName}\u201d wants to open a URL`, `MCP 服务器“${serverName}”想要打开一个链接`)} subtitle={`\n${message}`} color="permission" onCancel={() => onResponse('cancel')} isCancelActive inputGuide={exitState_0 => exitState_0.pending ? <Text>{uiText(`Press ${exitState_0.keyName} again to exit`, `再次按 ${exitState_0.keyName} 退出`)}</Text> : <Byline>
            <ConfigurableShortcutHint action="confirm:no" context="Confirmation" fallback="Esc" description={uiText('cancel', '取消')} />
            <KeyboardShortcutHint shortcut="\u2190\u2192" action={uiText('switch', '切换')} />
          </Byline>}>
      <Box flexDirection="column">
